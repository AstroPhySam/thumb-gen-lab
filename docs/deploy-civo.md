...
The thumbnail service is deployed to a single Civo instance with Terraform, Ansible and GitHub Actions. The Terraform root module lives in `infra/civo/`; the app deploy playbooks live in `ansible/` (`setup.yml`, `deploy.civo.yml`).

## Target architecture

```
GitHub Actions ──> Civo instance (Ubuntu 24.04, g4s.small)
...
- DNS/TLS is optional with a domain pointed at the instance, Caddy auto-provisions
  Let's Encrypt certs; with a bare IP it serves plain HTTP (`auto_https off`).

## Prerequisites (one-time)

1. **Civo account** : API key from the Civo dashboard: `CIVO_TOKEN`.
2. **Object Store** : create the `infrastates` bucket (S3-compatible) and an access/secret key pair for Terraform state.
3. **Two SSH keypairs**:
   - **Admin key** used by `civo-server-setup` (`root` bootstrap / `admin` re-runs).
   - **Deploy key** used by `civo-app-deploy` to connect as the `deploy` user (no sudo).
...
| secret   | `SSH_PUBLIC_KEY`         | public admin key, installed into `admin`'s authorized_keys                      |
...
1. **`civo-terraform-plan`** : `terraform fmt/init/validate/plan`.
2. **`civo-terraform-apply`** : `terraform apply -auto-approve`, then records the instance IP into the `INSTANCE_IP` repo variable.
3. **`civo-server-setup`** with `ssh_user = root` : creates `admin`/`deploy` users, hardens SSH, installs fail2ban, bootstraps rootless Docker. Re-runs use `ssh_user = admin`.
4. **`civo-app-deploy`** : clones the repo to `/opt/thumbgen`, renders the prod `.env`, builds + starts the stack via rootless Docker, then health-gates `http://<ip>/healthz`.
...
- fail2ban sshd jail (ban 10m after 5 failures). SSH stays open to `0.0.0.0/0` because GitHub Actions runners use dynamic IPs.
